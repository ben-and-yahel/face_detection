...
Well, our code takes an image and detects where the face area in the image and who is it.
...
- While we humans look for similarity in faces by eye size, nose position... this method is using a completely different way.
- We will train a computer to find himself the most 128 accurate measurements that unique the face, this model was trained on over 3 million faces and took 48 hours to understand which is the best way to measure face.
- This method is called **deep learning** when we do not understand how the computer got his result.
- As you can see in the image below we have a woman that her face is translated to 128 different measurements.
...
# stage 2.5 - find the similarity
- Ok so we extract the features from the face but now how can we know which is similar and which different.
- For that we will use clustering, by the DBSCAN method because it is relatively simple and fit for as.
- DBSCAN takes data and centers it into groups by their similarity.
...
- Our model was trained on 62*47 matrix and he can only recognize a face in this size.
- what we've done to solve this is to give our model the image in different sizes and search for different size faces in the image. 
# stage 3.0 - GUI (Graphic User Interface)
Well the last and most fun part is the building of our GUI.
We build an interface that can communicate using a socket with python and communicate with firebase so we can build our users so they will be cloud base.

# bottom line
In this project we faced a ton of bags, problems and obstacles and still had fun from every moment and learn a lot about everything we wanted and even we didn't get the best model or product we are very proud of what we have done and learn.
This project made us learn a lot about our self and what can we produce if we very want something.
